		...
		private void SetMainWinTitle()
		{
			...
			switch (TimerType)
			{
				case TimerType.EMOTM:
					title = string.Format("E{0}MO{1}M", (LengthOfMinute > 1 ? LengthOfMinute.ToString() : string.Empty),
						(LengthOfMinute > 1 ? LengthOfMinute.ToString() : "T"));
					break;
